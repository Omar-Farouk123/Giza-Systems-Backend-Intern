...
public class RequestService {
    ...
    public void populateRequests() {
        ...
        for (int i = 1; i < 2; i++)
            for (int j = 1; j <= 100; j++) {
                empno = (j % 100) + 1;
                Requests requests = new Requests(j, employeeRepo.getReferenceById(empno), 1 + "/8/2024", ((int) (Math.random() * 31) + 1) + "/8/2024", "31/7/2024", "app");
    ...
    public void updateRequests(){
        Requests requests=requestRepo.getReferenceById(4);
        requests.setStatus("ay7aga");
        requestRepo.save(requests);
    }
}
